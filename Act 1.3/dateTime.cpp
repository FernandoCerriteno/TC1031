...
int dateTime::busquedaBinaria(vector<Entrada> lista,int size,dateTime key){
  int l=0;
  int r=size-1;
  int m;
  while(l<=r){
    m=l+(r-l)/2;
    dateTime temp(lista[m].getMes(),lista[m].getDia(),lista[m].getHora(),lista[m].getMin(),lista[m].getSeg());
    if (key==temp){
      return m;
    }
    else if (key < temp){ 
